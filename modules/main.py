...
async def account_login(bot: Client, m: Message):
    ...
    try:
        await bot.send_document(
            ...
            document=x,
            caption=f"**📤 New File Uploaded by [{m.from_user.first_name}](tg://user?id={m.from_user.id})**"
        )
        await m.reply_text(f"📂 File sent to **BHOSDIKE BTANA JRURI H KYA** !")

        # Remove file from bot's storage after sending
        os.remove(x)
    except Exception as e:
        await m.reply_text(f"❌ Failed to send the file to channel: {str(e)}")
        if os.path.exists(x):
